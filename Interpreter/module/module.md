...
**PyModuleDef** is optional. The index located in the m_base field is used to find the module by index in sys.modules, not by name.

**m_size** stores the size of per-module data.

**m_clear** and **m_free** are used for deallocation.
...
When you try to look into the source code to find out how `import` works, follow the call stack
...
Searching for `_frozen_importlib`, you can find the half-binary file `Python/importlib.h` with content
...
The core opcode here is `IMPORT_NAME`.

It's tedious to copy and paste all the source code related to `IMPORT_NAME`, even with annotation.

Imagine that there are two or more threads currently importing the same `_locale` module. How does CPython handle this situation?

If you read the source code and the following images, you will notice that the lock mechanism is used for preventing race conditions.
...
There are currently three different finders in my `sys.meta_path`
...
`BuiltinImporter` will handle all the built-in modules. When we call 
