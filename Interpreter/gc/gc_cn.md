...
实际上 **分代回收机制** 是 **分代回收** 和 **标记清楚** 的结合, 这里我为了命名方便就叫做 **分代回收机制**
...
如果当前对象复制出的引用计数  > 0, 那么对于当前对象所引用到的, 所有其他的在用一个 **generation** 中的对象, 如果这个其他对象的复制引用计数 <= 0, 直接把它设置成 1, 如果这个对象在 **unreachable** 中, 把它移回 
**generation** 中
